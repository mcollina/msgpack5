...
const SIZES = { 0xc4: 2, 0xc5: 3, 0xc6: 5, 0xc7: 3, 0xc8: 4, 0xc9: 6, 0xca: 5, 0xcb: 9, 0xcc: 2, 0xcd: 3, 0xce: 5, 0xcf: 9, 0xd0: 2, 0xd1: 3, 0xd2: 5, 0xd3: 9, 0xd4: 3, 0xd5: 4, 0xd6: 6, 0xd7: 10, 0xd8: 18, 0xd9: 2, 0xda: 3, 0xdb: 5, 0xde: 3 }
...
module.exports = function buildDecode (decodingTypes) {
  ...
  function tryDecode (buf, initialOffset = 0) {
    ...
    // if (inRange(0x00, 0x7f)) TODO positive fixint
    // if (inRange(0x80, 0x8f)) TODO fixmap
    // if (inRange(0x90, 0x9f)) TODO fixarr
    // if (inRange(0xa0, 0xbf)) TODO fixstr
    ...
    // if (inRange(0xde, 0xdf)) return decodeMap(...)
    // if (inRange(0xe0, 0xff)) TODO negative fixint

    function getLength (buf, offset, size) {
      if (size === 1) return buf.readUInt8(offset) // up to 2^8 - 1 bytes
      if (size === 2) return buf.readUInt16BE(offset) // up to 2^16 - 2 bytes
      if (size === 4) return buf.readUInt32BE(offset) // up to 2^32 - 4 bytes
      // ERROR
    }

    function getType (buf, offset) {
      return buf.readUInt8(offset)
    }

    function decodeStr (buf, offset, size) {
      const length = getLength(buf, offset, size)
      offset += size
      if (!isValidDataSize(length, bufLength, size + 1)) return null

      result = buf.toString('utf8', offset, offset + length)

      return buildDecodeResult(result, size + 1 + length)
    }

    function decodeBuffers (buf, offset, size) {
      var length = getLength(buf, offset, size)
      offset += size
      if (!isValidDataSize(length, bufLength, size + 1)) return null
    ...
    function decodeExts (buf, offset, size) {
      ...
      const type = getType(buf, offset)
      offset += 1

      if (!isValidDataSize(length, bufLength, size + 2)) return null
      return decodeExt(buf, offset, type, length, size + 2, decodingTypes)
    }
  ...
  function decodeFixExts (buf, offset, size) {
    ...
    return decodeExt(buf, offset, type, size, 2, decodingTypes)
  }
}
...
function decodeExt (buf, offset, type, size, headerSize, decodingTypes) {

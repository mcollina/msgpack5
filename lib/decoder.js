...
module.exports = function buildDecode (decodingTypes) {
  ...
  function tryDecode (buf, initialOffset = 0) {
    ...
    if (first < 0x80) return buildDecodeResult(first, 1) // 7-bits positive ints
    ...
    if (first >= 0xe0) return buildDecodeResult(first - 0x100, 1) // 5 bits negative ints
    ...
    if ((first & 0xf0) === 0x90) {
      ...
      return decodeArray(buf, offset, first & 0x0f, 1)
    }

...
module.exports = function buildDecode (decodingTypes) {
  ...
  function tryDecode (buf, initialOffset = 0) {
    ...
    // if (inRange(0xc4, 0xc6)) return decodeBuffers(...)
    ...
    // if (inRange(0xdc, 0xdd)) return decodeArray(...)
    ...
    function decodeStr (buf, offset, size) {
      ...
      if (size === 1) length = buf.readUInt8(offset) // strings up to 2^8 - 1 bytes
      if (size === 2) length = buf.readUInt16BE(offset) // strings up to 2^16 - 2 bytes
      if (size === 4) length = buf.readUInt32BE(offset) // strings up to 2^32 - 4 bytes
